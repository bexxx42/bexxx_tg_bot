...
async def empty_query(query: types.InlineQuery):
    await query.answer(results=inline_results_python)


@dp.inline_handler(text="telegram")
async def empty_query(query: types.InlineQuery):
    await query.answer(results=inline_results_telegram)
    ...
    await query.answer(
        results=[
            types.InlineQueryResultArticle(
                ...
                thumb_url="https://i1.wp.com/mohirdev.uz/wp-content/uploads/Telegram-bot.png",
                description="Dasturlash asoslarini eng mashxur dasturlash tili - Pythonda o'rganamiz"
            ),
            types.InlineQueryResultArticle(
                id="kurs002",
